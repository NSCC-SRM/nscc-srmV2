...
export default function Domains() {
  return (
    <div
      className="min-h-screen bg-[#10131a] text-white bg-cover bg-center"
      style={{ backgroundImage: "url('/bg.png')" }}
    >
      <h2 className="text-4xl font-medium text-left px-[270px] sm:px-20 max-md:px-5 pt-16 pb-8 max-md:text-2xl">
      ...
      <div className="w-[90vw] max-w-[1200px] mx-auto bg-white/5 backdrop-blur-lg rounded-[28px] p-2 space-y-5">
        {domains.map((domain, i) => (
          <div
            key={i}
            className="flex flex-row max-md:flex-col items-center justify-between bg-transparent p-4 rounded-[28px] min-h-[180px] relative group hover:border-[2.5px] hover:border-teal-400 transition duration-300"
          >
            {/* Description */}
            <div className="w-[38%] ml-12 text-gray-300 text-base leading-relaxed line-clamp-4 group-hover:text-white max-md:w-full max-md:ml-0 max-md:mt-2 max-md:text-sm">
              {domain.desc}
            </div>

            {/* Thumbnail Preview (hidden until lg) */}
            <div className="flex-1 lg:flex hidden items-center justify-center relative min-w-[180px] min-h-[180px]">
            ...
            {/* Domain Name */}
            <div
              className={`w-[28%] text-right pr-16 text-4xl font-extrabold leading-tight tracking-tight max-md:w-full max-md:text-left max-md:pr-0 max-md:pt-2 ${domain.color}`}
            >
              {domain.name}
            </div>
          </div>
